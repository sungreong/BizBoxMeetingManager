...
class Schedule(QWidget):
    def __init__(self):
        ...
        login_hbox.addWidget(id_label)
        login_hbox.addWidget(self.id_text)
        pw_label = QLabel("PW:")
        self.pw_text = PasswordEdit(self)
        login_hbox.addWidget(pw_label)
        login_hbox.addWidget(self.pw_text)
        if "USER" in self.config:
            self.pw_text.setText(self.config["USER"].get("pw", ""))
            self.id_text.setText(self.config["USER"].get("id", ""))
        else:
            self.pw_text.setPlaceholderText("PW 입력")
            self.id_text.setPlaceholderText("ID 입력")

        self.login_btn = QPushButton("접속", self)
        self.login_btn.setGeometry(300, 60, 100, 30)
        self.login_btn.clicked.connect(self.login)
        login_hbox.addWidget(self.login_btn)
        self.login_widget.setLayout(login_hbox)
        ...
        self.room_combo.addItems(
            ["2F) 자율좌석 회의실", "2F) 대회의실", "2F) 소회의실", "3F) 대회의실", "3F) 소회의실", "3F) 중회의실", "Zoom1", "Zoom2"]
        )
        ...
        self.filter_combo.addItems(
            [
                ...
                "3F) 소회의실",
                "3F) 중회의실",
                "Zoom1 : agilesoda@agilesoda.ai",
                "Zoom2 : agilesoda2@agilesoda.ai",
            ]
        )
        ...
        pregress_hbox = QHBoxLayout()
        self.progress_bar = QProgressBar(self)
        pregress_hbox.addWidget(self.progress_bar)
        self.progress_bar.hide()
        # QVBoxLayout로 QHBoxLayout을 세로로 배치
        vbox = QVBoxLayout()
        vbox.addWidget(self.login_widget)
        ...
        QMessageBox.information(self, "알림", "아이디와 패스워드를 입력해주시고, 접속 버튼을 클릭해주세요.")

    def login(self):
        try:
            self.bizbox = BizBoxCommon(user=self.id_text.text(), pw=self.pw_text.text())
            self.bizbox.add_progressbar(self.progress_bar)
            self.open_bizbox()
        except Exception as e:
            return QMessageBox.warning(self, "알림(오류)", "아이디와 패스워드를 다시 입력해주세요.")
        else:
            self.login_widget.setVisible(False)
            if os.path.exists("./info.ini"):
                print("info.ini exists")
            else:
                if QMessageBox.question(self, "알림", "아이디와 패스워드를 info.ini에 저장하시겠습니까?") == QMessageBox.Yes:
                    config = configparser.ConfigParser()
                    # ini 파일 읽어들이기
                    config.add_section("USER")
                    config.set("USER", "id", self.id_text.text())
                    config.set("USER", "pw", self.pw_text.text())
                    with open("./info.ini", "w") as configfile:
                        config.write(configfile)
                    QMessageBox.information(self, "알림", "info.ini에 저장되었습니다. 해당 프로그램을 전달할 때 주의하세요!")

            return QMessageBox.warning(self, "알림(오류)", "접속이 완료되었습니다. 아래 버튼들을 클릭하여, 회의실을 조회 및 등록해주세요.")
        ...
        for row in range(self.table.rowCount()):
            ...
            if len(remain_meeting_room_list) == 0:
                return QMessageBox.information(
                    self, f"알림", f"{start_time}~{end_time} 시간대에 남은 회의실은 없습니다. 다른 시간대로 예약해주세요."
                )
            else:
                return QMessageBox.information(
                    self, f"알림", f"{start_time}~{end_time} 시간대에 남은 회의실은 {', '.join(remain_meeting_room_list)} 입니다."
                )
    ...
    def view_schedule(self):
        ...
        try:
            ...
            self.progress_bar.setMaximum(len(df_room))
